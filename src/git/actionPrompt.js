...
const useTerminalSize_1 = __importDefault(require("./useTerminalSize"));
// Set your box size here
const ACTION_HEIGHT = 8;
const ACTION_WIDTH = 16;
const bel = () => process.stdout.write("\x07");
const gitActions = (0, core_1.createPrompt)((config, done) => {
    const [position, setPosition] = (0, core_1.useState)([0, 0]);
    const [selectSubActionIndex, setSelectSubActionIndex] = (0, core_1.useState)(-1);
    // eslint-disable-next-line no-unused-vars
const { width, height } = (0, useTerminalSize_1.default)();
    // eslint-disable-next-line no-unused-vars
    ...
    (0, core_1.useKeypress)(
        // eslint-disable-next-line require-yield
        (key) => __awaiter(void 0, void 0, void 0, function* () {
            var _a, _b;
            if (selectSubActionIndex !== -1) {
                ...
                //@ts-expect-error meta is not defined in type but exists in the event
                    ...
                    const action = (_a = grid[position[0]][position[1]].subActions) === null || _a === void 0 ? void 0 : _a[selectSubActionIndex];
                    if (action) {
                        done(action.value);
                    }
                }
                    ...
                    const actionWithKey = (_b = grid[position[0]][position[1]].subActions) === null || _b === void 0 ? void 0 : _b.find(a => a.key && a.key.toLowerCase() === key.name);
                    if (actionWithKey) {
...
exports.default = gitActions;
function renderActionBox(action, isSelected, selectSubActionIndex = -1, width, height) {
    ...
    if (isSelected) {
        for (let i = 0; i < height; i++) {
            lines[i] = lines[i].replace(/./g, (ch) => `\x1b[7m${ch}\x1b[0m`);
        }
    }
    return lines;
}
